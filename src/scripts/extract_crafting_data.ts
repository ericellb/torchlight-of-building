 ...
 */
const parseAffixText = (td: cheerio.Cheerio<cheerio.Element>, $: cheerio.CheerioAPI): string => {
...
const extractCraftingData = (html: string): CraftingAffix[] => {
  ...
  const fixedHtml = html.replace('<tbody>', '<table><tbody>').replace('</tbody>', '</tbody></table>');
...
const main = async () => {
  try {
    ...
    const htmlPath = join(process.cwd(), '.garbage', 'crafting', 'codex_crafting.html');
    ...
    console.log(`✓ Successfully wrote ${affixes.length} affixes to ${outputPath}`);
  } catch (error) {
